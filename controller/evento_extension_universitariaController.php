...
class evento_extension_universitariaController extends Controller {
    ...
    public function index() {
        ...
        $data = array();
        $seme=$this->mostrar_semestre_ultimo();
        $datos = $data['data'] = $obj->index($_GET['q'], $_GET['p'], $_GET['criterio'],$_SESSION['idusuario'],$seme);
        $data['query'] = $_GET['q'];
...
public function pdf_eu() {
        $idevento=$_POST['idevento'];
        $envio=$this->ListaPdf_eu($idevento);
      echo $envio;  
       }
    ...
    public function edit() {
        ...
        $data['tipo_evento'] = $this->Select(array('id' => 'idtipo_evento', 'name' => 'idtipo_evento', 'table' => 'tipo_evento where tipo_evento.idtipo_evento="6"', 'code' => $obj['idtipo_evento']));
//        $data['sub_eventos'] = $this->leer_sub_eventos($_GET['id'],  $obj['idtipo_evento'], $obj['UBIGEO']);
...
//,"UNIRSE"
    ...
    public function index_alumno() {
        ...
        $data = array();
        $seme=$this->mostrar_semestre_ultimo();
        $datos = $data['data'] = $obj->index1($_GET['q'], $_GET['p'], $_GET['criterio'],$_SESSION['idusuario'],$seme);
        $data['query'] = $_GET['q'];
        $data['pag'] = $this->Pagination(array('rows' => $data['data']['rowspag'], 'url' => 'index.php?controller=evento_extension_universitaria&action=index', 'query' => $_GET['q']));
...
//,"UNIRSE"
        $opt = array("evento_extension_universitaria.DescripcionTipoColegio" => "Tema");
        $data['grilla'] = $this->grilla_E_PS_EU("evento_extension_universitaria", $cols, $data['data']['rows'], $opt, $data['pag'], false, false, false, FALSE, true, false, true);
        $view = new View();
        $view->setData($data);
        $view->setTemplate('../view/evento_extension_universitaria/_Index.php');
        $view->setLayout('../template/Layout3.php');
    ...
    public function create() {
        $data = array();
        $view = new View();
        $data['tipo_evento'] = $this->Select(array('id' => 'idtipo_evento', 'name' => 'idtipo_evento', 'table' => 'tipo_evento where tipo_evento.idtipo_evento="6"'));
